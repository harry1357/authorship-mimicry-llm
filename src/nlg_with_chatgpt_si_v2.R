...
for (author in list.of.directories) {
    ...
    if (simple.or.complex == "simple") {
        existing.file<-paste('../chatgpt_generated_texts_ftc/simple/', author, "_", batch.n,
    ...
    if (file.exists(existing.file)) {
        ...
        if (simple.or.complex == "simple") {
            ...
            write.file.name<-paste('../chatgpt_generated_texts_ftc/simple_study/', author, "_", batch.n, '_gpt_model_',
                                   gpt.model, '_top_p_', top.p.value, '_', topics, '.txt', sep="")
            writeLines(out.text[1], write.file.name)

            write.file.name<-paste('../chatgpt_generated_texts_ftc/simple/', author, '_', batch.n, '_gpt_model_',
                                   gpt.model, '_top_p_', top.p.value, '_', topics, '.txt', sep="")
